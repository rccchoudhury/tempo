...
from .kpt3d_mview_rgb_direct_dataset import Kpt3dMviewRgbImgDirectDataset


@DATASETS.register_module()
class Body3DMviewTemporalShelfDataset(Kpt3dMviewRgbImgDirectDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode:
            result = self._prepare_test_sample(idx)
        else:
            result = self._prepare_train_sample(idx)
    ...
    def _prepare_test_sample(self, idx):
        ...
        for cam_id, cam_param in self.cameras.items():
            ...
            for person in range(self.num_persons):
                pose3d = self.gt_pose_db[0][person][fid][0] * 1000.0
                if len(pose3d[0]) > 0:
            ...
            # Compute num persons and roots 3D for inference.
            all_poses_3d = np.array(all_poses_3d)
            num_persons = all_poses_3d.shape[0]

            roots_3d = np.zeros((self.max_nposes, 3))
            if num_persons > 0:
                roots = np.mean(all_poses_3d[:, self.root_id], axis=1)
                roots_3d[:num_persons] = roots
    ...
    def evaluate(self,
        ...
        for idx, fid in enumerate(self.frame_range):
            pred_coco = pose_3ds[idx].copy()
            ...
            for person in range(self.num_persons):
                gt = self.gt_pose_db[0][person][fid][0] * 1000.0
                if len(gt[0]) == 0:
                    continue
    ...
    @staticmethod
    def panoptic2shelf3D(panoptic_pose, alpha=0.75):
        """transform panoptic order(our method output) 3d pose to shelf dataset
        order with interpolation."""
        ...
        # L and R shoulder
        mid_sho = (panoptic_pose[3] + panoptic_pose[9]) / 2
        # middle of two ear
        head_center = (panoptic_pose[17] + panoptic_pose[18]) / 2
        ...
        shelf_pose[12] = (shelf_pose[8] + shelf_pose[9]) / 2
        # use nose to init
        shelf_pose[13] = panoptic_pose[1]

        shelf_pose[13] = shelf_pose[12] + (
            shelf_pose[13] - shelf_pose[12]) * np.array([0.75, 0.75, 1.5])
        shelf_pose[12] = shelf_pose[12] + (
            panoptic_pose[1] - shelf_pose[12]) * np.array([0.5, 0.5, 0.5])
