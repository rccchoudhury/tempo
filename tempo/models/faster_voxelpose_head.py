...
class BEVCenterHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 max_num=10,
                 max_pool_kernel=3):
        super(BEVCenterHead, self).__init__()
        # use register_buffer
        #self.register_buffer('grid_size', torch.tensor(space_size))
        #self.register_buffer('cube_size', torch.tensor(cube_size))
        #Eself.register_buffer('grid_center', torch.tensor(space_center))

        self.num_candidates = max_num
        self.max_pool_kernel = max_pool_kernel
        self.loss = nn.MSELoss()

    def _get_real_locations(self, indices, grid_size, grid_center, cube_size):
        """
        Args:
    ...
    def _nms_by_max_pool(self, heatmap_volumes):
        ...
        topk_unravel_index = self.get_2d_indices(topk_flatten_index,
    ...
    def forward(self, heatmap_volumes, space_size, space_center, cube_size):
        """
        ...
        """
        batch_size = heatmap_volumes.shape[0]
        space_size = torch.tensor(space_size, device=heatmap_volumes.device)
        space_center = torch.tensor(
            space_center, device=heatmap_volumes.device)
        cube_size = torch.tensor(cube_size, device=heatmap_volumes.device)

        topk_values, topk_unravel_index, _ = self._nms_by_max_pool(
    ...
    def get_loss(self, center_heatmaps_2d, targets_2d, center_heatmaps_1d,
                 targets_1d):
        return dict(
            loss_2d=self.loss(center_heatmaps_2d, targets_2d),
            loss_1d=self.loss(center_heatmaps_1d, targets_1d))


@HEADS.register_module()
class FasterCuboidPoseHead(nn.Module):
    ...
    def __init__(self, beta):
        ...
        """
        super(FasterCuboidPoseHead, self).__init__()
        self.beta = beta
    ...
    def forward(self, heatmap_volumes, grid_coordinates):
        ...
        channel = heatmap_volumes.size(2)
        heatmap_volumes = heatmap_volumes.reshape(batch_size, 3, channel, -1,
                                                  1)
        heatmap_volumes = F.softmax(self.beta * heatmap_volumes, dim=3)

        # confidence score: averaged max values in the joint feature map
        confs, _ = torch.max(heatmap_volumes, dim=3)
        confs = torch.mean(confs.squeeze(3), dim=(1, 2))
        grid_coordinates = grid_coordinates.unsqueeze(2)
        human_poses = torch.mul(heatmap_volumes, grid_coordinates)
        human_poses = torch.sum(human_poses, dim=3)

        return human_poses, confs

    def get_loss(self, preds, preds2d, targets, weights):

        loss = self.loss(preds2d[:, 0] * weights, targets[:, :, :2] * weights)
        loss += self.loss(preds2d[:, 1] * weights,
                          targets[:, :, ::2] * weights)
        loss += self.loss(preds2d[:, 2] * weights, targets[:, :, 1:] * weights)
        loss += 2 * self.loss(preds * weights, targets * weights)
        return dict(loss_pose=loss)
